...
local function configure_lang_servers(lsp_zero)
  ...
  -- handling java lsp
  tryCatch(setupJdtlsPlugin, errorHandlerForJdtls)
end
...
local function setupLspZeroPlugin()
...
local function errorHandlerForLspZero(error)
  vim.notify('Lsp plugin could not be loaded!', WARN)
  vim.notify(error, ERROR)
end

tryCatch(setupLspZeroPlugin, errorHandlerForLspZero)
